class Pord{
    ...
    static def initItem(caller) {
        ...
        IMAGE_TAG         = "3.${caller.BUILD_NUMBER}." + (Math.random() * 900 +100).intValue().toString() + "-p-SNAPSHOT"
        ...
        caller.dir("${APP_NAME}") {
            if( ! caller.fileExists('chart') ){
                ...
                caller.dir("charts/${APP_NAME}") {
                    ...
                    //修改启动数量 replicaCount: 1
                    caller.sh "sed -i -e 's/replicaCount:\n1/replicaCount:\n${REPLICA_COUNT}/' values.yaml"
                    //修改 cpu 和 内存参数
    ...
    static def predestroy(caller){
        ...
        ${GIT_REPO}  ${APP_NAME}   ${BRANCH_NAME}  发布成功,发布数量： ${REPLICA_COUNT}
        =================================================================================================================================
        """
    }
